...
| `NetProbe.java` | 無料・キー不要の外部API照合（IP / メール / 逆ジオコーディング） |
...
| 用途 | エンドポイント | 使いどころ |
|---|---|---|
| IP情報 | `https://ipapi.co/json/` | グローバルIP・国・地域・回線事業者・ASN |
| メール検証 | `https://disify.com/api/email/<addr>` | 形式 / 使い捨て / MX(DNS) |
| 逆ジオコーディング | `https://api.bigdatacloud.net/data/reverse-geocode-client` | 緯度経度→日本語の地名 |

- **接続元の国**: 想定国コード（`home_country`、既定JP）と不一致なら weight15 のNG。渡航・VPN経由・認証情報漏えいの検知に使う。
- **接続経路**: 回線事業者名にデータセンター/VPN事業者を示す語が含まれれば weight8 のNG（`HOSTING_HINTS`）。無料APIには proxy/vpn 判定フィールドが無いため事業者名からの推定であることに注意。
- **IP照合の鮮度**: 7日以上前なら weight5 のNG。`DailyWorker` が日次で自動更新し、結果は `ipinfo.json` に保存、エクスポートJSONの「接続元IP情報」にも載る。
